...
export default class ModalidadesController {
  public async index({ request, response }: HttpContextContract) {
    ...
    if (combo) {
      return response.send({
        ...
        payload: await prisma.modalidad.findMany({
          orderBy: [
            { posicion: 'asc' },
            { nombre: 'asc' },
          ],
  ...
  public async store({ request, response }: HttpContextContract) {
    const payload = await request.validate({
      ...
      messages: {
        'posicion.range': 'El rango válido es de 0 a 2147483647',
      },
    })
    if ((await prisma.modalidad.count({
      where: {
        nombre: {
    ...
    try {
      return response.send({
        ...
        payload: await prisma.modalidad.create({
          data: payload,
        }),
      })
    } catch(err) {
  ...
  public async show({ request, response }: HttpContextContract) {
    try {
      return response.send({
        ...
        payload: await prisma.modalidad.findUniqueOrThrow({
          where: {
            id: request.param('id'),
          },
        }),
  ...
  public async update({ request, response }: HttpContextContract) {
    ...
    try {
      return response.send({
        ...
        payload: await prisma.modalidad.update({
          where: {
            id: request.param('id'),
          },
          data: payload,
